...
export function ProfilesModal({ isOpen = true, onClose }: ProfilesModalProps) {
  ...
  const handleOverwrite = async (profile: ModProfile) => {
    await saveCurrentProfile(profile.name, profile.description, profile.id);
        ...
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-surface/60 hover:bg-surface text-textMuted hover:text-textMain border border-white/5 text-xs font-bold transition-colors flex items-center gap-1.5 cursor-pointer"
            >
              <Download size={13} />
              {t('profile_export')}
            </button>
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-surface/80 hover:bg-surface text-textMain text-xs font-bold transition-colors border border-white/10 cursor-pointer"
          >
            {t('close')}
          </button>
        </div>
        ...
        >
          ...
          <div className="flex justify-end">
            <button
              ...
              className="px-5 py-2 rounded-xl bg-primary text-white text-xs font-bold hover:bg-primary/80 transition-all flex items-center gap-2 shadow-md shadow-primary/20 disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed"
            >
          ...
          {showImportBox && (
            ...
            >
              <textarea
                ...
                placeholder={t('profile_import_placeholder')}
                rows={4}
              ...
              <div className="flex justify-end gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-textMuted hover:text-textMain text-xs font-bold cursor-pointer"
                >
                  {t('cancel')}
                </button>
                <button
                  ...
                  className="px-4 py-1.5 rounded-lg bg-primary text-white text-xs font-bold hover:bg-primary/80 cursor-pointer"
                >
                  {t('profile_import')}
                </button>
              </div>
        ...
        <div className="space-y-3">
          {filteredProfiles.length === 0 ? (
            <div className="text-center py-10 text-textMuted space-y-2">
              ...
              <p className="text-xs opacity-70">{t('profile_none_hint')}</p>
            </div>
          ) : (
              ...
              return (
                ...
                >
                  <div className="space-y-1 min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-textMain text-sm truncate">{prof.name}</span>
                      {isActive && (
                    ...
                    <div className="flex items-center gap-3 text-[11px] text-textMuted opacity-80 pt-0.5">
                      <span className="flex items-center gap-1">
                        <Layers size={12} />
                        {t('profile_mods_count', { count: prof.enabledModRelativePaths.length })}
                      </span>
                      <span className="flex items-center gap-1">
                        <Calendar size={12} />
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-primary text-white text-xs font-bold hover:bg-primary/80 transition-all flex items-center gap-1.5 shadow-sm shadow-primary/20 disabled:opacity-50 cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-surface/60 hover:bg-surface text-textMuted hover:text-textMain border border-white/5 text-xs transition-colors cursor-pointer"
